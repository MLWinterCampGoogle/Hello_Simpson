...
# path_ds_image = tf.data.Dataset.from_tensor_slices(all_image_paths)
# label_ds_image = tf.data.Dataset.from_tensor_slices(all_matting_paths)

# image_label_ds = tf.data.Dataset.zip((path_ds_image, label_ds_image))
...
# train = image_label_ds.map(load_image_train, num_parallel_calls=tf.data.experimental.AUTOTUNE)
# test = image_label_ds.map(load_image_test)
...
def show_predictions(dataset=None, num=1, epoch=0):
  if dataset:
    ...
    output_image.save('Prediction_text_filename.jpg')

    # matting_process = tf.io.read_file('./matting/1803251855/matting_00000000/1803251855-00000274.png')

    # matting_process = tf.image.decode_png(matting_process)
    matting_process = cv2.imread('./matting/1803251855/matting_00000000/1803251855-00000274.png', cv2.IMREAD_UNCHANGED)

    matting_process = cv2.resize(matting_process, (IMG_SIZE, IMG_SIZE))

    if tf.random.uniform(()) > 0.5:
      matting_process = tf.image.flip_left_right(matting_process)

    matting_process = normalize(matting_process, False)

    # matting_process = tf.reshape(matting_process, [IMG_SIZE, IMG_SIZE, 1])
    mat = np.reshape(matting_process,(128,128))

    # Creates PIL image
    cv2.imwrite('matting_process.jpg', mat)
